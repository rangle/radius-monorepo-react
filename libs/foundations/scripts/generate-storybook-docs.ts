...
import { TokenLayer, TokenLayers, TokenOutput } from './lib/token-parser.types';
...
const renderLayerDescription = (layer: TokenLayer | undefined) => {
  ...
  if (parameterNames.length === 0)
    return `
- \`static layer\` **[${name}](?path=/docs/about-token-documentation-${name.replace(
      '--',
      '-'
    )}-design-tokens--docs)** 
    ...
    ${description}
      `;
  return `
- \`dynamic layer\` **[${name}](?path=/docs/about-token-documentation-${name.replace(
    '--',
    '-'
  )}-design-tokens--docs)**
...
const writeLayerDefinitions = async (
  fileName: string,
  storyName: string,
  order: string[],
  layers: TokenLayer[],
...
const baseTokenDefinitions = (name: string, variables) => {
  ...
  return [tokenHeader(name), layerVariables].join('\n');
};

const writeTokenDefinitions = async (
  name: string,
  baseTokenDefinitions: string,
...
if (!tokenFileName) {
  console.error(`Usage:  ts-node ${__filename} <tokenFileName>`);
  process.exit(1);
}
...
readTokenData(tokenFileName).then((tokenFile) => {
  ...
  let fileCount = 4;
  writeLayerDefinitions(
    `${STORYBOOK_DOCS_PATH}/${fileIndex(
      fileCount++
    )}-token-layer-definitions.mdx`,
    'Layer Definition',
    order,
    layers,
    layerMap
